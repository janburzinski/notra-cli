type FlagKind = 'string' | 'integer' | 'boolean';

type FlagDefinition<Value = unknown> = {
  kind: FlagKind;
  description?: string;
  required?: boolean;
...
type CommandConstructor = typeof Command & {
  args?: Record<string, ArgDefinition>;
  flags?: Record<string, FlagDefinition<unknown>>;
};

type InferArgs<Definition> = Definition extends Record<string, ArgDefinition>
  ? { [Key in keyof Definition]: Definition[Key] extends { options: readonly (infer Value extends string)[] }
      ? Definition[Key] extends { required: true } ? Value : Value | undefined
      : Definition[Key] extends { required: true } ? string : string | undefined }
  : Record<never, never>;

type InferFlags<Definition> = Definition extends Record<string, FlagDefinition<unknown>>
  ? { [Key in keyof Definition]: Definition[Key] extends FlagDefinition<infer Value>
...
type ParsedCommand<Constructor> = {
  args: Constructor extends { args: infer Definition } ? InferArgs<Definition> : Record<never, never>;
  flags: Constructor extends { flags: infer Definition } ? InferFlags<Definition> : Record<never, never>;
};
